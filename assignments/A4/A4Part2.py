...
def computeSNR(inputFile, window, M, N, H):
    ...
    """
    # get input data
    (fs, Input) = UF.wavread(inputFile)
    
    inSignal = Input[M : len(Input) - M]

    # get window type
    w = get_window(window, M, False)
    
    # compute stft, get signal resynthesized
    reSynth = stft.stft(Input, w, N, H)
    
    # signal with M samples removed from beggining and end 
    signal = reSynth[M : len(reSynth) - M]

    noise = np.arange(float(len(Input)))
    for i in range(len(Input)):
        noise[i] = Input[i] - reSynth[i]
    
    noise2 = np.arange(float(len(signal)))
    for i in range(len(signal)):
        noise2[i] = inSignal[i] - signal[i]


    noiseEnergy = energy(noise)
    noiseEnergy2 = energy(noise2)

    Energy1 = energy(Input)
    Energy1 += energy(reSynth)

    Energy2 = energy(signal)

    Energy1dB = amp2dB(Energy1)
    Energy2dB = amp2dB(Energy2)
    noiseEndB = amp2dB(noiseEnergy)
    noise2EndB = amp2dB(noiseEnergy2)

    snr1 = 10 * np.log10(Energy1/noiseEnergy)
    snr2 = 10 * np.log10(Energy2/noiseEnergy2)
