...
def findMinima(array, start_index):
    ant = i = start_index
    
    while(i < len(array) - 1):
        start_index += 1
        if (array[start_index] < array[ant]):
           ant = start_index
        else:
            break
        i += 1 
...
def extractMainLobe(window, M):
    ...
    mainlobe = np.arange(float(minima*2+1))
    mainlobe[:minima] = mX1[int(N/2)-minima:int(N/2)]
    mainlobe[minima:] = mX1[int(N/2):int(N/2)+minima+1]
