...
def computeODF(inputFile, window, M, N, H):
    ...
    ## Code here
    OnsetLow = np.arange(float(len(energyLow_dB)))
    i = 1
    while (i < len(energyLow_dB)):
        OnsetLow[i] = energyLow_dB[i] - energyLow_dB[i - 1]
        i += 1
    
    OnsetLow[OnsetLow < 0] = 0.0
    
    OnsetHigh = np.arange(float(len(energyHigh_dB)))
    i = 1
    while (i < len(energyHigh_dB)):
        OnsetHigh[i] = energyHigh_dB[i] - energyHigh_dB[i - 1]
        i += 1
    
    OnsetHigh[OnsetHigh < 0] = 0.0

    returnArr = np.ndarray([len(OnsetLow), 2], float)  
