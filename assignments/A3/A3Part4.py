...
def genSine(A, f, phi, fs, t):
    ...
    for i in range(int(nSamples +1)):
        x[i] = A * np.cos((2 * np.pi * f * i / fs) + phi)
...
def suppressFreqDFTmodel(x, fs, N):
    ...
    filtIndex = int(np.ceil((70.0 * N/2.0)/(fs/2.0)))
    #filtIndex = int(np.ceil((70.0 * N/2.0)/fs))
    mY = mX.copy()

    mX[:filtIndex + 1] = -120.0
    
    y =  dftSynth(mY, pX, w.size) * outputScaleFactor

    yfilt = dftSynth(mX, pX, w.size) * outputScaleFactor

    #y1 = dftModel(y, w, N)
    #yfilter2 = dftModel(yfilter, w, N)
